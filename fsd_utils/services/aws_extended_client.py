...
class SQSExtendedClient:
    ...
    def submit_single_message(
        ...
        self.logger.info("Called SQS and submitted the message and id [%s]", message_id)
        return message_id
    ...
    def receive_messages(self, queue_url, max_number, visibility_time=1, wait_time=1):
        ...
        if "Messages" in response.keys():
            ...
            self.logger.info("Called SQS and received [%s] messages", len(messages))
    ...
    def _delete_msg_from_sqs_and_s3(
        ...
        if "Successful" in response:
            for msg_meta in response["Successful"]:
                self.logger.info(
                    "Deleted %s", reciept_handles_to_delete[int(msg_meta["Id"])]
                )
        if "Failed" in response:
            for msg_meta in response["Failed"]:
                self.logger.info(
                    "Could not delete %s",
                    reciept_handles_to_delete[int(msg_meta["Id"])],
                )
        return response
    ...
    def _delete_message_from_s3(self, messages, reciept_handles_to_delete):
        for msg in messages:
            ...
            self.logger.info(
                "NOSUCHKEYDEBUG: about to DeleteObject on {s3_bucket_name}/{s3_key}",
                extra=dict(s3_bucket_name=s3_bucket_name, s3_key=s3_key),
            )
            ...
            if status_code != 204:
                self.logger.info(
                    "NOSUCHKEYDEBUG: failed to DeleteObject on {s3_bucket_name}/{s3_key}",
                    extra=dict(s3_bucket_name=s3_bucket_name, s3_key=s3_key),
                )
                raise SQSExtendedClientException(
                    ExceptionMessages.FAILED_DELETE_MESSAGE.format(status_code)
            ...
            self.logger.info(
                "NOSUCHKEYDEBUG: successful DeleteObject on {s3_bucket_name}/{s3_key}",
                extra=dict(s3_bucket_name=s3_bucket_name, s3_key=s3_key),
            )
    ...
    def _retrieve_message_from_s3(self, message_body: str) -> str:
        ...
        s3_bucket_name, s3_key = s3_details["s3BucketName"], s3_details["s3Key"]
        self.logger.info(
            "NOSUCHKEYDEBUG: about to GetObject on {s3_bucket_name}/{s3_key}",
            extra=dict(s3_bucket_name=s3_bucket_name, s3_key=s3_key),
        )
        ...
        if status_code != 200:
            self.logger.info(
                "NOSUCHKEYDEBUG: failed to GetObject on {s3_bucket_name}/{s3_key}",
                extra=dict(s3_bucket_name=s3_bucket_name, s3_key=s3_key),
            )
            raise SQSExtendedClientException(
                ExceptionMessages.FAILED_RECEIVE_MESSAGE.format(status_code)
        ...
        self.logger.info(
            "NOSUCHKEYDEBUG: successful GetObject on {s3_bucket_name}/{s3_key}",
            extra=dict(s3_bucket_name=s3_bucket_name, s3_key=s3_key),
        )
    ...
    def _store_message_in_s3(
        ...
        if self.large_payload_support and self.always_through_s3:
            ...
            self.logger.info(
                "Message added to S3 bucket [%s]", self.large_payload_support
            )
    ...
    def _get_queue_names(self, queues):
        if queues:
            self.logger.info("Got queues: %s", ", ".join([q for q in queues]))
