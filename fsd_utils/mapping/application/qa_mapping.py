...
from fsd_utils.mapping.application.languages import EN, SUPPORTED_LANGUAGES
from fsd_utils.mapping.application.multi_input import MultiInput


def extract_questions_and_answers(forms, language=EN) -> dict:  # noqa: C901
    ...
    try:
        for count, form in enumerate(forms, start=1):
            form_name = form["name"]
            current_app.logger.info("Form %s) %s", count, form_name)
                    ...
                    for field in question["fields"]:
                        ...
                        if field["type"] == "file":
                            ...
                            if isinstance(answer, str):
                                questions_answers[form_name][field["title"]] = answer.split("/")[-1]
                            else:
                                questions_answers[form_name][field["title"]] = answer

                        elif isinstance(answer, bool) and field["type"] == "list":
                            questions_answers[form_name][field["title"]] = YES[language] if answer else NO[language]

                        elif isinstance(answer, list) and field["type"] == "multiInput":
                            questions_answers[form_name][field["title"]] = MultiInput.map_multi_input_data(
                                answer, language
                            )
                            ...
                            questions_answers[form_name][field["title"]] = clean_html_answer

                        elif isinstance(answer, list) and field["type"] == "list":
                            questions_answers[form_name][field["title"]] = format_checkbox(answer)

                        elif isinstance(answer, str) and field["type"] == "list":
                            questions_answers[form_name][field["title"]] = format_radio_field(answer)

                        elif field["type"] == "monthYear":
                            questions_answers[form_name][field["title"]] = format_month_year(answer)

                        elif field["type"] == "date":
                            questions_answers[form_name][field["title"]] = format_date_month_year(answer)
        ...
        current_app.logger.error("Error occurred while processing form data: %s", e)
        current_app.logger.error("Could not map the data for form: %s", form_name)
        raise Exception(f"Could not map the data for form: {form_name}") from e
